...
\title{SNORT\textsuperscript{\textregistered} Users Manual\\2.9.4}
...
\item Int Blklst = packets that are GTP or Teredo encapsulated that are being
blocked.  These packets could get the Blacklist verdict if \texttt{config
tunnel\_verdicts} was set for the given protocol.  Note that this count are
...
\item Int Whtlst = packets that are GTP or Teredo encapsulated that are being
allowed.  These packets could get the Whitelist verdict if \texttt{config
tunnel\_verdicts} was set for the given protocol.  Note that this count are
...
Minimum value is 1 and the maximum is 65535.  The default is 10.  For a given
...
\hline \texttt{config tunnel\_verdicts: gtp|teredo} & By default, whitelist and
blacklist verdicts are handled internally by Snort for GTP and Teredo
...
tunnel.  If your DAQ decodes GTP or Teredo correctly, setting this config
